...
###################################################################################################
    ...
    mri_slices=[]
    prediction_slices=[]
    for n in range(temp_frame.shape[2]):
        ...
        buffer.seek(0)
        mri_slices.append(b64encode(buffer.read()).decode('utf-8'))
        ...
        plt.savefig(buffer, format='png', bbox_inches='tight', pad_inches=0)
        buffer.seek(0)
        prediction_slices.append(b64encode(buffer.read()).decode('utf-8'))

        # Close the plot to free up memory
        plt.close()

    return render_template('Segmentation.html',mri_slices=mri_slices, prediction_slices=prediction_slices)
